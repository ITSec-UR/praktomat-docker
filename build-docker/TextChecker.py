...
class TextChecker(Checker):
    ...
    def _remove_multiblock_comments_with_line_map(self, text: str, log_messages: list[str]) -> tuple[list[str], list[int]]:
        ...
        if len(symbols) % 2 != 0:
            log_messages.append(
                "<p><em>Warnung:</em> Ungültige Einstellungen in 'multi_block'. "
                "Es müssen Paare von Start- und Endsymbolen sein, z.&nbsp;B. <code>/* */</code>.</p>"
            )
            lines = text.splitlines()
            return lines, list(range(1, len(lines) + 1))
    ...
    def run(self, env):
                ...
                for pattern, norm_pattern in normalized_patterns.items():
                    ...
                    if self.use_regex:
                        try:
                            ...
                            log_messages.append(
                                f"<p><em>Fehler:</em> Ungültiger regulärer Ausdruck: "
                                f"'<code>{escape(pattern)}</code>'.</p>"
                            )
                            continue
                    else:
                        start = 0
        ...
        for pattern in patterns:
            ...
            if must_be_present and not has_hits:
                ...
                passed = 0
                log.append(self._format_message("Fehler", f"'<code>{escape(pattern)}</code>' sollte nicht vorkommen, wurde aber gefunden:"))
                log.append(self._format_hits(hits))
            else:
            ...
            if self.use_regex and explanations[pattern]:
                log.append("<p><em>Details:</em></p>")
                log.append("<ul>" + ''.join(explanations[pattern]) + "</ul>")
